...
import type { PlatformStatus, HealthStatus, AnalyticsSummary, EventsData } from "@/lib/api";

/* ============================================================
   Platform colors (static, for display only)
   ============================================================ */
...
function platformColor(name: string) {
  return PLATFORM_COLORS[name.toLowerCase().replace(/ .*/, "")] || "#888";
}
...
function ActivityIcon({ type }: { type: string }) {
  const paths: Record<string, string> = {
    ...
    raid: "M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z",
  };
  return (
...
export default function AdminDashboard() {
  ...
  const [recentEvents, setRecentEvents] = useState<EventsData["recent"]>([]);
  const [revenue, setRevenue] = useState<{ donations: number; subscriptions: number; merchandise: number } | null>(null);
  const [coreUptime, setCoreUptime] = useState(0);
  const [liveStatus, setLiveStatus] = useState({ isLive: false, liveCount: 0 });
  ...
  const fetchData = async () => {
    try {
      const [platformsRes, healthRes, liveRes, analyticsRes, eventsRes, revenueRes, coreRes] = await Promise.allSettled([
        api.platforms(),
        api.health(),
        api.live(),
        api.analyticsSummary(30),
        api.analyticsEvents(7),
        api.analyticsRevenue(30),
        api.status(),
      ]);

      if (platformsRes.status === "fulfilled") setPlatforms(platformsRes.value.platforms);
      if (healthRes.status === "fulfilled") setHealth(healthRes.value);
      if (liveRes.status === "fulfilled") setLiveStatus(liveRes.value);
      if (analyticsRes.status === "fulfilled" && !analyticsRes.value.error) setAnalytics(analyticsRes.value);
      if (eventsRes.status === "fulfilled") setRecentEvents(eventsRes.value.recent || []);
      if (revenueRes.status === "fulfilled") {
        setRevenue({
          donations: revenueRes.value.donations || 0,
          subscriptions: revenueRes.value.subscriptions || 0,
          merchandise: revenueRes.value.merchandise || 0,
        });
      }
      if (coreRes.status === "fulfilled") setCoreUptime(coreRes.value.uptime);
  ...
  const allOnline = health?.core?.status === "operational";

  // Build live stats from real data
  const liveStats = [
    {
      label: "Platforms Live",
      value: String(liveStatus.liveCount),
      change: liveStatus.isLive ? "LIVE" : "Offline",
      changeColor: liveStatus.isLive ? "text-emerald" : "text-dim",
      icon: "M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z M15 12a3 3 0 11-6 0 3 3 0 016 0z",
    },
    {
      label: "Messages (30d)",
      value: analytics ? analytics.messages.total.toLocaleString() : "--",
      change: analytics ? `${analytics.messages.change > 0 ? "+" : ""}${analytics.messages.change}%` : "--",
      changeColor: analytics && analytics.messages.change >= 0 ? "text-emerald" : "text-error",
      icon: "M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z",
    },
    {
      label: "Followers (30d)",
      value: analytics ? analytics.followers.total.toLocaleString() : "--",
      change: analytics ? `${analytics.followers.change > 0 ? "+" : ""}${analytics.followers.change}%` : "--",
      changeColor: analytics && analytics.followers.change >= 0 ? "text-emerald" : "text-error",
    ...
    },
    {
      ...
      change: analytics ? `${analytics.revenue.change > 0 ? "+" : ""}${analytics.revenue.change}%` : "--",
      changeColor: analytics && analytics.revenue.change >= 0 ? "text-emerald" : "text-error",
      icon: "M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z",
    },
  ];

  // System health items from real data
  const systemHealth = [
    {
      name: "PRISMAI Core",
      status: health?.core?.status === "operational" ? "online" : "offline",
      detail: coreUptime ? `Uptime: ${formatUptime(coreUptime)}` : "--",
      version: health?.core?.version || "--",
    },
    {
      name: "Analytics Server",
      status: health?.analytics?.status === "operational" ? "online" : health?.analytics?.status === "offline" ? "offline" : "standby",
      detail: health?.analytics?.version || "--",
      version: "",
    },
    {
      name: "Connected Platforms",
      status: connectedCount >= 8 ? "online" : connectedCount >= 5 ? "standby" : "offline",
      detail: `${connectedCount} / ${platforms.length}`,
      version: "",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            {loading ? "Connecting to PRISMAI..." : apiError ? `Connection issue: ${apiError}` : "Live ecosystem overview"}
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          {liveStatus.isLive && (
            <div className="badge badge-live">
              ...
              Live on {liveStatus.liveCount} platforms
            </div>
          )}
          <div className={`badge ${allOnline ? "badge-emerald" : "badge-red"}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${allOnline ? "bg-emerald" : "bg-error"}`} />
            {allOnline ? "All Systems Go" : "Issues Detected"}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {liveStats.map((stat) => (
          <div key={stat.label} className="card p-5 group">
            ...
            <div className="text-2xl font-extrabold tracking-tight">{loading ? "..." : stat.value}</div>
      ...
      {analytics?.sparkline && analytics.sparkline.length > 0 && (
        <div className="card p-4">
          <div className="text-label text-dim mb-2">Message Activity (30 days)</div>
          <div className="flex items-end gap-1 h-16">
            {analytics.sparkline.map((val, i) => {
              const max = Math.max(...analytics.sparkline, 1);
              return (
                <div
                  key={i}
                  className="flex-1 rounded-t bg-gradient-to-t from-red/40 to-red-bright/60 transition-all hover:from-red/60 hover:to-red-bright/80"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
              {platforms.length === 0 && loading ? (
                <div className="col-span-full text-center text-sm text-dim py-8">Loading platforms...</div>
              ) : (
                platforms.map((platform) => {
                  const color = platformColor(platform.name);
                  return (
                    <div
                      key={platform.name}
        ...
        {/* Recent Activity (from real events API) */}
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
          ...
          <div className="space-y-1">
            {recentEvents.length === 0 && !loading ? (
              <div className="text-center text-sm text-dim py-8">No recent events</div>
            ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium truncate">
                        ...
                        <span className="text-dim">
                          {" "}{event.type === "follow" ? "followed" : event.type === "sub" ? "subscribed" : event.type === "donation" ? `donated${event.amount ? ` $${event.amount}` : ""}` : event.type === "order" ? "placed order" : event.type === "raid" ? "raided" : "event"}
                        </span>
                      </div>
                      <div className="text-[10px] text-dim">{event.platform} &middot; {timeAgo(event.timestamp)}</div>
                    </div>
                  </div>
                );
              })
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="powered-by-prismai">
            ...
            PRISMAI Engine {health?.core?.version ? `v${health.core.version}` : ""}
          </div>
        </div>
          ...
          {systemHealth.map((service) => (
            ...
            >
              ...
              <div className="text-right">
                <div className="text-data text-xs">{service.detail}</div>
                {service.version && <div className="text-[10px] text-dim">v{service.version}</div>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold">Revenue (30 days)</h2>
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div>
              <div className="text-xl font-extrabold text-gold">
                {revenue ? `$${revenue.donations.toLocaleString()}` : "--"}
              </div>
              <div className="text-[10px] text-dim">Tips & Donations</div>
            </div>
            <div>
              <div className="text-xl font-extrabold text-electric">
                {revenue ? `$${revenue.subscriptions.toLocaleString()}` : "--"}
              </div>
              <div className="text-[10px] text-dim">Subscriptions</div>
            </div>
            <div>
              <div className="text-xl font-extrabold text-emerald">
                {revenue ? `$${revenue.merchandise.toLocaleString()}` : "--"}
              </div>
              <div className="text-[10px] text-dim">Merch Sales</div>
            </div>
          </div>
          {analytics?.sparkline && analytics.sparkline.length > 0 && (
            <div className="h-32 rounded-lg bg-glass border border-glass-border flex items-center justify-center p-3">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold">Stream Status</h2>
            <Link href="/admin/stream" className="text-xs text-muted hover:text-foreground transition-colors">
              Manage Stream
              <svg className="w-3 h-3 inline ml-1" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
              </svg>
          ...
          {liveStatus.isLive ? (
            <>
              <div className="flex items-center gap-3 mb-4 p-4 rounded-lg bg-red/10 border border-red/20">
                <span className="live-dot" />
                <div>
                  <div className="text-sm font-bold text-red-bright">Currently Live</div>
                  <div className="text-xs text-muted">Broadcasting on {liveStatus.liveCount} platform{liveStatus.liveCount !== 1 ? "s" : ""}</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {platforms.filter(p => p.live).map(p => (
                  <div key={p.name} className="p-3 rounded-lg bg-glass flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-red-bright animate-pulse-live" />
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <div className="text-sm font-semibold mb-1">Not Currently Live</div>
              <div className="text-xs text-dim">{connectedCount} platforms connected and ready</div>
            </div>
          )}
        </div>
      </div>
