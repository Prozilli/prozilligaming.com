...
type AlertType = "live" | "offline" | "clip";
...
const DEFAULT_ALERTS: Record<AlertType, AlertConfig> = {
  live: {
    ...
    embedEnabled: true,
    embedColor: "#53FC18",
    title: "🔴 {streamer} is LIVE on Kick!",
    description: "**{title}**\nPlaying {game}\n\n[Watch Now]({url})",
    showThumbnail: true,
    showGame: true,
    showViewers: false,
  ...
  offline: {
    ...
    embedColor: "#7a8899",
    title: "Stream Ended",
    description: "{streamer} has gone offline on Kick. Thanks for watching!",
    showThumbnail: false,
    showGame: false,
    showViewers: false,
  },
  clip: {
    ...
    description: "Check out this clip from {streamer}!\n\n[Watch Clip]({url})",
    showThumbnail: true,
    showGame: true,
    showViewers: false,
  },
...
export default function KickAlertsPage() {
  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        const savedAlerts = await getSetting<Record<AlertType, AlertConfig>>("kick_alerts");
        if (savedAlerts) {
  ...
  const handleSave = async () => {
    ...
    setSaveStatus("idle");
    try {
      await saveSetting("kick_alerts", alerts);
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-3">
            <svg className="h-7 w-7 text-[#53FC18]" viewBox="0 0 24 24" fill="currentColor">
              <path d="M1.333 0v24h5.338V14.666L12 20l5.333-5.334V24h5.334V0h-5.334v9.334L12 4 6.671 9.334V0z" />
            </svg>
            Kick Alerts
          </h1>
          <p className="mt-1 text-sm text-gray-400">
            Notify your Discord when you go live, post clips, and more on Kick
          </p>
        </div>
        <div className="flex items-center gap-3 rounded-lg bg-[#53FC18]/10 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#53FC18] flex items-center justify-center text-black font-bold text-sm">
            P
          </div>
          <div>
            <p className="text-sm font-medium text-white">{connectedChannel}</p>
            <p className="text-[10px] text-[#53FC18]">Connected</p>
          </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {activeAlertType === "live"
                    ? "Send a message when you start streaming on Kick"
                    : activeAlertType === "offline"
                    ? "Send a message when you stop streaming on Kick"
                    : "Post new Kick clips to Discord"}
                </p>
              </div>
              <button
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="rounded-lg bg-[#53FC18] px-6 py-2.5 text-sm font-medium text-black hover:bg-[#53FC18]/90 disabled:opacity-50"
            >
                ...
                {currentAlert.embedEnabled ? (
                  ...
                  >
                    {currentAlert.showThumbnail && (
                      <div className="h-32 bg-gradient-to-r from-[#53FC18]/20 to-[#53FC18]/10 flex items-center justify-center">
                    ...
                    <div className="p-3">
                      ...
                      <p className="mt-2 text-sm text-gray-300 whitespace-pre-wrap">
                        {currentAlert.description
                          ...
                          .replace("{url}", "https://kick.com/prozilligaming")
                          .replace("{viewers}", "142")}
                      </p>
