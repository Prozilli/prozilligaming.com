...
type AlertType = "video" | "live" | "offline" | "short";
...
const DEFAULT_ALERTS: Record<AlertType, AlertConfig> = {
  video: {
    ...
    description: "**{channel}** just uploaded a new video!\n\nDuration: {duration}\n\n[Watch Now]({url})",
    showThumbnail: true,
    showGame: false,
    showViewers: false,
  },
  live: {
    ...
    embedEnabled: true,
    embedColor: "#FF0000",
    title: "🔴 {channel} is LIVE on YouTube!",
    description: "**{title}**\n\n[Watch Now]({url})",
    showThumbnail: true,
    showGame: false,
    showViewers: false,
  ...
  offline: {
    ...
    embedColor: "#7a8899",
    title: "Stream Ended",
    description: "{channel} has gone offline on YouTube. Thanks for watching!",
    showThumbnail: false,
    showGame: false,
    showViewers: false,
  },
  short: {
    ...
    description: "**{channel}** posted a new Short!\n\n[Watch Now]({url})",
    showThumbnail: true,
    showGame: false,
    showViewers: false,
  },
...
export default function YouTubeAlertsPage() {
  ...
  const [connectedChannel] = useState("Prozilli Gaming");
  ...
  useEffect(() => {
    async function loadSettings() {
      try {
        const savedAlerts = await getSetting<Record<AlertType, AlertConfig>>("youtube_alerts");
        if (savedAlerts) {
  ...
  const handleSave = async () => {
    ...
    setSaveStatus("idle");
    try {
      await saveSetting("youtube_alerts", alerts);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            Notify your Discord about new videos, live streams, and shorts
          </p>
        </div>
        <div className="flex items-center gap-3 rounded-lg bg-[#FF0000]/10 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#FF0000] flex items-center justify-center text-white font-bold text-sm">
            P
          </div>
          <div>
            <p className="text-sm font-medium text-white">{connectedChannel}</p>
            <p className="text-[10px] text-[#FF0000]">Connected</p>
          </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { id: "short", label: "New Short", icon: "📱", enabled: alerts.short.enabled },
        ].map((type) => (
          <button
            key={type.id}
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {activeAlertType === "video"
                    ? "Send a message when you upload a new video"
                    : activeAlertType === "live"
                    ? "Send a message when you start streaming on YouTube"
                    : activeAlertType === "offline"
                    ? "Send a message when you stop streaming on YouTube"
                    : "Send a message when you post a new YouTube Short"}
                </p>
              </div>
              <button
                onClick={() => updateAlert("enabled", !currentAlert.enabled)}
                className={`relative h-6 w-11 rounded-full transition-colors ${
                  currentAlert.enabled ? "bg-[#FF0000]" : "bg-raised"
                }`}
              >
                <span
                  ...
                  <div>
                    ...
                    {channelsLoading ? (
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border border-[var(--color-border)] bg-surface px-4 py-2.5 text-sm text-white focus:border-[#FF0000] focus:outline-none"
                      >
              ...
              <div className="rounded-xl border border-[var(--color-border)] bg-surface p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className={`relative h-6 w-11 rounded-full transition-colors ${
                      currentAlert.embedEnabled ? "bg-[#FF0000]" : "bg-raised"
                    }`}
                  >
                    <span
                      className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <input
                          ...
                          className="flex-1 rounded-lg border border-[var(--color-border)] bg-surface px-4 py-2.5 text-sm text-white focus:border-[#FF0000] focus:outline-none"
                        />
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={4}
                        className="w-full rounded-lg border border-[var(--color-border)] bg-surface px-4 py-2.5 text-sm text-white focus:border-[#FF0000] focus:outline-none resize-none"
                      />
                    </div>
                    <div className="flex flex-wrap gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="h-4 w-4 rounded border-white/20 bg-surface text-[#FF0000] focus:ring-[#FF0000]"
                        />
              ...
              <div className="rounded-xl border border-[var(--color-border)] bg-surface p-5">
                ...
                <div className="grid gap-2 text-xs">
                  <div className="flex justify-between">
                    <code className="text-[#FF0000]">{"{channel}"}</code>
                    <span className="text-gray-500">Channel name</span>
                  </div>
                  <div className="flex justify-between">
                    <code className="text-[#FF0000]">{"{title}"}</code>
                    <span className="text-gray-500">Video/stream title</span>
                  </div>
                  <div className="flex justify-between">
                    <code className="text-[#FF0000]">{"{url}"}</code>
                    <span className="text-gray-500">Link to video/stream</span>
                  </div>
                  <div className="flex justify-between">
                    <code className="text-[#FF0000]">{"{viewers}"}</code>
                    <span className="text-gray-500">Current viewer count</span>
                  </div>
                  <div className="flex justify-between">
                    <code className="text-[#FF0000]">{"{duration}"}</code>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="rounded-lg bg-[#FF0000] px-6 py-2.5 text-sm font-medium text-white hover:bg-[#FF0000]/90 disabled:opacity-50"
            >
                ...
                {currentAlert.embedEnabled ? (
                  ...
                  >
                    {currentAlert.showThumbnail && (
                      <div className="h-32 bg-gradient-to-r from-[#FF0000]/20 to-[#FF0000]/10 flex items-center justify-center">
                    ...
                    <div className="p-3">
                      <p className="font-semibold text-white text-sm">
                        {currentAlert.title
                          .replace("{channel}", connectedChannel)
                          .replace("{title}", "Top 10 Gaming Moments This Week!")}
                      </p>
                      <p className="mt-2 text-sm text-gray-300 whitespace-pre-wrap">
                        {currentAlert.description
                          .replace("{channel}", connectedChannel)
                          .replace("{title}", "Top 10 Gaming Moments This Week!")
                          .replace("{url}", "https://youtube.com/watch?v=example")
                          .replace("{viewers}", "1,024")
                          .replace("{duration}", "12:34")}
                      </p>
